...
final_diameter = st.slider("Final Hole Diameter (mm)", 0.5, 20.0, 10.0, step=0.05)
cone_diameter = st.slider("Cone Opening Diameter (mm)", final_diameter + 0.1, 25.0, min(final_diameter + 5.0, 25.0))
channel_length = st.slider("Channel (Land) Length (mm)", 0.1, min(35.0, plate_thickness), 10.0, step=0.1)
...
arc_length = 2 * np.pi * (20 + (number_of_rows - 1) * space_between_rows / 2)
space_between_holes = (arc_length / holes_per_row) - final_diameter
...
st.write(f"Space Between Holes (edge-to-edge): {space_between_holes:.2f} mm")
...
cone = patches.Polygon([
    ...
    [final_diameter/2, cone_length],
    [cone_diameter/2, 0]
], closed=True, facecolor='lightblue', edgecolor='black', label='Cone Section')
ax.add_patch(cone)

# Channel section
channel = patches.Rectangle((-final_diameter/2, cone_length), final_diameter, channel_length,
                            facecolor='lightgreen', edgecolor='black', label='Channel Section')
